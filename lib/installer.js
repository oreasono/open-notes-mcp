...
const TOKEN_ADDED_STRUCTURED = "# open-notes-mcp:token-budget:added-enabled";
const TOKEN_CHANGED = "# open-notes-mcp:token-budget:changed:";
const TOKEN_STRUCTURED_CHANGED = "# open-notes-mcp:token-budget:changed-enabled:";
const GUIDANCE_ADDED = "# open-notes-mcp:guidance:added";
const GUIDANCE_CHANGED = "# open-notes-mcp:guidance:changed:";
const GUIDANCE_MARKER = "open-notes-mcp:guidance:v1";
const GUIDANCE_CATALOG_SKIP = "SKIP guidance: model catalog owns token-budget defaults (see openai/codex#42918)";
const GUIDANCE_MESSAGE = [
  GUIDANCE_MARKER,
  "Before calling `new_context`, update `INDEX.md`; `new_context` does not save anything.",
  "Keep `INDEX.md` as a table of contents, not a notebook: record a filename, one-line summary, and pointer per topic; put details in partition files.",
  "Update `INDEX.md` at task boundaries: conclusions, direction changes, and completed steps.",
].join("\n");
...
init asks before enabling features.token_budget for the Codex harness. The
token-budget flags provide an explicit non-interactive answer. Guidance is
enabled by default only for custom model providers; the guidance flags provide
...
function modelProviderEntry(config) {
  const { lines } = splitLines(config);
  let table = "";
  let entry = null;
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (tableMatch) {
      table = normalizedTableName(tableMatch[1]);
      continue;
    }
    if (table !== "") continue;
    const match = line.match(/^\s*model_provider\s*=\s*("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')\s*(?:#.*)?$/);
    if (!match) continue;
    if (entry) throw new Error("Codex config defines model_provider more than once");
    let value;
    if (match[1].startsWith('"')) {
      try {
        value = JSON.parse(match[1]);
      } catch {
        throw new Error("Codex config has an invalid model_provider string");
      }
    } else {
      value = match[1].slice(1, -1).replace(/\\(['\\])/g, "$1");
    }
    entry = { index, line, value };
  }
  return entry;
}

function isCustomModelProvider(config) {
  const provider = modelProviderEntry(config)?.value;
  return typeof provider === "string" && provider.trim().length > 0 && provider.trim().toLowerCase() !== "openai";
}

function shouldInstallGuidance(config, guidanceChoice = null) {
  if (guidanceChoice === true) return true;
  if (guidanceChoice === false) return false;
  return isCustomModelProvider(config);
}
...
function inlineTableParts(line, key) {
  const match = line.match(new RegExp(`^(\\s*)${key.replace(".", "\\.")}\\s*=\\s*\\{(.*)\\}(\\s+#.*)?$`));
  if (!match) return null;
  return { indent: match[1], body: match[2], comment: match[3] || "" };
}

function inlineHasGuidance(line, key) {
  const parts = inlineTableParts(line, key);
  return Boolean(parts && /(?:^|,)\s*guidance_message\s*=/.test(parts.body));
}
...
function tokenBudgetEntry(config) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    } else if (table === "features.token_budget") {
      match = line.match(/^(\s*)enabled\s*=\s*(true|false)\s*(?:#.*)?$/);
      if (match) installedLine = `${match[1]}enabled = true`;
    }
    if (match) entries.push({
      ...
      line,
      installedLine,
      kind: table === "features" ? "table" : table === "" ? "dotted" : "structured",
      tableStart: table === "features.token_budget" ? index : undefined,
    });
    const inlineKey = table === "features" ? "token_budget" : table === "" ? "features.token_budget" : null;
    const inline = inlineKey && inlineTableParts(line, inlineKey);
    if (inline) {
      const enabled = inline.body.match(/(?:^|,)\s*enabled\s*=\s*(true|false)\b/);
      entries.push({
        index,
        value: enabled?.[1] === "true",
        line,
        installedLine: line,
        kind: table === "features" ? "inline-table" : "inline-dotted",
        inlineKey,
      });
    }
  }
...
function enableTokenBudget(config) {
  ...
  if (current) {
    if (current.kind === "structured") {
      if (current.installedLine === current.line) {
        lines.splice(current.index + 1, 0, TOKEN_ADDED_STRUCTURED, "enabled = true");
      } else {
        const encoded = Buffer.from(current.line, "utf8").toString("base64url");
        lines.splice(current.index, 1, `${TOKEN_STRUCTURED_CHANGED}${encoded}`, "enabled = true");
      }
      return lines.join(newline);
    }
    if (current.kind === "inline-table" || current.kind === "inline-dotted") {
      const encoded = Buffer.from(current.line, "utf8").toString("base64url");
      const updated = current.line.replace(/(\benabled\s*=\s*)false\b/, "$1true");
      lines.splice(current.index, 1, `${TOKEN_CHANGED}${encoded}`, updated);
      return lines.join(newline);
    }
    const encoded = Buffer.from(current.line, "utf8").toString("base64url");
...
function removeManagedTokenBudget(config) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index];
    if (line === TOKEN_ADDED_STRUCTURED) {
      if (lines[index + 1] === "enabled = true") lines.splice(index, 2);
      else lines.splice(index, 1);
      return lines.join(newline);
    }
    if (line.startsWith(TOKEN_STRUCTURED_CHANGED)) {
      const encoded = line.slice(TOKEN_STRUCTURED_CHANGED.length);
      const original = Buffer.from(encoded, "base64url").toString("utf8");
      if (lines[index + 1] === "enabled = true") lines.splice(index, 2, original);
      else lines.splice(index, 1);
      return lines.join(newline);
    }
...
function guidanceMessageEntry(config) {
  const { lines } = splitLines(config);
  let table = "";
  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index];
    ...
    if (table === "features.token_budget" && /^\s*guidance_message\s*=/.test(line)) {
      return { kind: "structured", index, line };
    }
    if (table === "" && /^\s*features\s*\.\s*token_budget\s*\.\s*guidance_message\s*=/.test(line)) {
      return { kind: "dotted", index, line };
    }
    if (table === "features" && inlineHasGuidance(line, "token_budget")) {
      return { kind: "inline-table", index, line };
    }
    if (table === "" && inlineHasGuidance(line, "features.token_budget")) {
      return { kind: "inline-dotted", index, line };
    }
  }
  return null;
}

function guidanceLine(indent = "") {
  return `${indent}guidance_message = ${tomlString(GUIDANCE_MESSAGE)}`;
}

function inlineGuidanceLine(line, key) {
  const parts = inlineTableParts(line, key);
  if (!parts) throw new Error("Codex config has an invalid token_budget inline table");
  const body = parts.body.trimEnd();
  const separator = body.trim() ? ", " : "";
  return `${parts.indent}${key} = {${body}${separator}guidance_message = ${tomlString(GUIDANCE_MESSAGE)}}${parts.comment}`;
}

function scalarGuidanceLine(line) {
  const match = line.match(/^(\s*)(token_budget|features\s*\.\s*token_budget)\s*=\s*true(\s+#.*)?$/);
  if (!match) throw new Error("Codex config has an invalid guidance ownership marker");
  const key = match[2].includes(".") ? "features.token_budget" : "token_budget";
  return `${match[1]}${key} = { enabled = true, guidance_message = ${tomlString(GUIDANCE_MESSAGE)} }${match[3] || ""}`;
}

function managedGuidanceLine(original) {
  const inline = original.match(/^(\s*)(token_budget|features\s*\.\s*token_budget)\s*=\s*\{/);
  if (inline) {
    const key = inline[2].includes(".") ? "features.token_budget" : "token_budget";
    return inlineGuidanceLine(original, key);
  }
  return scalarGuidanceLine(original);
}

function upsertGuidanceMessage(config) {
  if (guidanceMessageEntry(config)) return { text: config, ownership: null };
  const token = tokenBudgetEntry(config);
  if (!token?.value) return { text: config, ownership: null };
  const { lines, newline } = splitLines(config);
  if (token.kind === "structured") {
    const line = guidanceLine();
    lines.splice(token.tableStart + 1, 0, GUIDANCE_ADDED, line);
    return { text: lines.join(newline), ownership: { kind: "added" } };
  }
  if (token.kind === "inline-table" || token.kind === "inline-dotted") {
    const key = token.inlineKey;
    const changed = inlineGuidanceLine(token.line, key);
    const encoded = Buffer.from(token.line, "utf8").toString("base64url");
    lines.splice(token.index, 1, `${GUIDANCE_CHANGED}${encoded}`, changed);
    return { text: lines.join(newline), ownership: { kind: "changed" } };
  }
  const encoded = Buffer.from(token.line, "utf8").toString("base64url");
  const key = token.kind === "table" ? "token_budget" : "features.token_budget";
  const changed = scalarGuidanceLine(token.line).replace(
    /^(\s*)(token_budget|features\.token_budget)\s*=/,
    `$1${key} =`,
  );
  lines.splice(token.index, 1, `${GUIDANCE_CHANGED}${encoded}`, changed);
  return { text: lines.join(newline), ownership: { kind: "changed" } };
...
function removeManagedGuidance(config) {
  const { lines, newline } = splitLines(config);
  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index];
    if (line === GUIDANCE_ADDED) {
      if (lines[index + 1] === guidanceLine()) lines.splice(index, 2);
      else lines.splice(index, 1);
      return lines.join(newline);
    }
    if (line.startsWith(GUIDANCE_CHANGED)) {
      const original = Buffer.from(line.slice(GUIDANCE_CHANGED.length), "base64url").toString("utf8");
      if (!original) throw new Error("Codex config has an invalid guidance ownership marker");
      const expected = managedGuidanceLine(original);
      if (lines[index + 1] === expected) lines.splice(index, 2, original);
      else lines.splice(index, 1);
      return lines.join(newline);
    }
  }
...
function parseInitOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--enable-token-budget") {
      ...
      tokenChoice = false;
    } else if (arg === "--with-guidance") {
      if (guidanceChoice === false) throw new Error("guidance options are mutually exclusive");
      guidanceChoice = true;
    } else if (arg === "--without-guidance") {
      if (guidanceChoice === true) throw new Error("guidance options are mutually exclusive");
      guidanceChoice = false;
    } else if (arg === "--harness") {
  ...
  return { tokenChoice, guidanceChoice, harness };
}
...
function prepareInit(loc, binaryPath, tokenChoice, guidanceChoice, state) {
  ...
  let guidance = null;
  const installGuidance = enableToken && shouldInstallGuidance(config, guidanceChoice);
  if (installGuidance) {
    const preparedGuidance = upsertGuidanceMessage(nextConfig);
    nextConfig = preparedGuidance.text;
    guidance = preparedGuidance.ownership;
  }
  ...
  return {
    ...
    guidanceChoice,
    guidanceMode: guidanceChoice === true ? "with" : guidanceChoice === false ? "without" : "auto",
    guidanceSkipped: enableToken && !installGuidance && guidanceChoice !== false,
    guidance,
    existingMcp,
    originalToken,
  };
...
function initCodex(tokenChoice, guidanceChoice, io = console, env = process.env) {
  ...
  const prepared = prepareInit(loc, loc.binaryPath, tokenChoice, guidanceChoice, state);
  ...
  try {
    ...
    const nextState = {
      ...
      guidanceMode: prepared.guidanceMode,
  ...
  if (prepared.guidanceSkipped) io.log(GUIDANCE_CATALOG_SKIP);
  else if (prepared.guidanceChoice === false) io.log("SKIP guidance: disabled by explicit choice");
...
function doctorCodex(io = console, env = process.env) {
  ...
  try {
    const token = tokenBudgetEntry(config);
    tokenEnabled = Boolean(token?.value);
    if (tokenEnabled) report("PASS", "token budget", "enabled");
    else {
      inactive = true;
  ...
  try {
    const guidance = guidanceMessageEntry(config);
    if (!tokenEnabled) {
      io.log("SKIP guidance: token_budget is disabled");
    } else if (!isCustomModelProvider(config) && state?.guidanceMode !== "with") {
      io.log(GUIDANCE_CATALOG_SKIP);
    } else if (guidance) {
      if (guidance.line.includes(GUIDANCE_MARKER)) {
        report("PASS", "guidance", "managed guidance_message installed");
      } else {
        report("PASS", "guidance", "user-owned guidance_message preserved");
      }
    } else if (state?.guidanceMode === "without") {
      io.log("SKIP guidance: disabled by explicit choice");
    } else {
      report("FAIL", "guidance", "no guidance_message configured");
    }
  } catch (error) {
    report("FAIL", "guidance", error.message);
  }
...
function uninstallCodex(io = console, env = process.env) {
  ...
  if (state) {
    ...
    nextConfig = removeManagedGuidance(nextConfig);
...
function init(args, io = console, env = process.env) {
  ...
  if (options.harness === "claude-code") {
    ...
    if (options.guidanceChoice !== null) throw new Error("guidance options apply only to the Codex harness");
    return initClaude(io, env);
  }
  return initCodex(options.tokenChoice, options.guidanceChoice, io, env);
}
